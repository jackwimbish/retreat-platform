...
const DefaultView = (props) => {
  ...
  // Check if we're on the homepage (Plone Site root)
  const isHomepage = content?.['@type'] === 'Plone Site' || path === '';

  // If the content is not yet loaded, then do not show anything
  return contentLoaded ? (
    <>
      {/* Add welcome banner for homepage */}
      {isHomepage && (
        <Container>
          <Message
            positive
          ...
          >
            <Message.Header>
              Welcome to the Retreat Experience Platform
            </Message.Header>
            <p style={{ marginTop: '1rem' }}>
              Your central hub for managing long-duration bootcamps, retreats,
              and residential events. This platform streamlines communication,
              scheduling, and facility management for participants and
              administrators.
            </p>
          </Message>
        </Container>
